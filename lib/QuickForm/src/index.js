    ...
    buttonGroupRender: {
      ...
      default: (value, submitFunc, resetFunc, createElement, validate) => {
        ...
        return [
          ...
          <el-button on-click={resetFunc}>重置</el-button>
        ];
      }
    },
  ...
  methods: {
    ...
    isMemberComponents(type) {
      ...
      memberComponents.map((conf, idx) => {
        if (!type) return;
        const findTypeIdx = _.indexOf(conf.type, type);
        if (findTypeIdx >= 0) {
    ...
    memberComponentsRender(memberType, NodeConf, idx, memberListIndex) {
      ...
      //  }
      const setDisabled = _.isUndefined(baseProps.disabled)
        ? this.disabled
      ...
      if (rules && rules.length && _.findIndex(rules, o => o.required) >= 0) {
        required = true;
      }
      const setProps = {
        key: idx,
          ...
          model: {
            ...
            callback: function($$v) {
              ...
              if (!valueFormula && formValue[name] != $$v) {
                // 当设置值的关联规则的时候，不能填写
                const changVal = _.cloneDeep(_this.formValue);
                changVal[name] = $$v;
                _this.formValue = changVal;

                _this.checkValueFormulaMap(_this.formValue);
                if (_.isFunction(afterItemValueChange)) {
                  afterItemValueChange(name, $$v, _this);
                }
                if (_.isFunction(afterValueChange)) {
                  afterValueChange(_this.formValue, _this);
                }
                if (_.isFunction(baseProps.afterChange)) {
    ...
    slotRender(NodeConf) {
      let render;
      if (_.isFunction(NodeConf.render)) {
    ...
    filterFormItem(item, idx, parentList) {
      ...
      let result = [];
      const isSolt = _.indexOf(["slot"], item.type) >= 0;
      let isGroup = false;

      if (item.valueFormula) { this.valueFormulaMap[item.name] = item.valueFormula; } // valueFormula用于后期关联表单值
      if (_.isEqual(item, {}) || _.isEqual(item, { inline: undefined })) {
        return this.renderEmpty();
      }
      if (_.isArray(item)) {
        // 假如表单项是数组，则自动将其配置成group
        const makeGroup = _.cloneDeep(baseGroupConf);

        makeGroup.childForm = item;

        item = makeGroup;
      }
      isGroup = _.indexOf(["group"], item.type) >= 0;
      ...
      switch (true) {
        ...
        case _.indexOf(["textNode"], item.type) >= 0:
          // 占位文本
          result = item.name
 ...
 */
    ...
    renderFieldWrap(itemConf) {
      ...
      const willSetFieldProps = {
        ...
        prop: prop || name,
        required:
          !!(rules && _.findIndex(rules, { required: true }) >= 0),
        rules,
        ...formItemProps
      };

      formItemContentRender = _.isFunction(formItemContentRender)
        ? formItemContentRender
    ...
    validSuccessAfterFunc(mergeData) {
      ...
      let setVal = _.cloneDeep(this.formValue.__ob__.value);
  ...
  render(h) {
    ...
    return (
      <div class="comp-quickform">
        ...
        {isSlotRender ? null : (
          <el-form {...{ attrs: formAttrs }} ref="form">
            <el-row gutter={20}>
              {this.formRender()}
              {buttonGroupColConfig ? (
                <el-col
