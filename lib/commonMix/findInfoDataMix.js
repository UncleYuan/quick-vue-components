...
export default {
  ...
  watch: {
    fetchData: function(newVal, oldVal) {
      if (!lodash.isEqual(newVal, oldVal) && !this.disabledFetch) {
  ...
  methods: {
    ...
    resetData(data = {}) {
      ...
      if (
        ...
        // 当数据是各自请求时合并
        this.infoData = { ...lodash.merge(this.infoData, data) };
      } else {
        this.infoData = data;
      }
    },
    ...
    makeConf(conf, index) {
      ...
      const setUrl = lodash.isFunction(conf.url)
      ...
      return http[fetchType](setUrl, fetchDataAll, setConf).then(res =>
        typeof index == "number"
