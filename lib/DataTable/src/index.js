...
export default {
  ...
  watch: {
    ...
    propData: function(newVal, oldVal) {
      if (this.ctrlData || (lodash.isArray(newVal) && !lodash.isEqual(newVal, oldVal))) {
    ...
    showHeaders() {
      return this.filterHeaders(this.tableHeader, o => {
        return {
          ...o,
          hidden:
            lodash.findIndex(this.showHeadersState, obj => obj == o.prop) < 0
        };
      });
    }
  },
  ...
  methods: {
    ...
    changeSelectRow(newVal, init) {
      if (this.$refs.table) {
        ...
        newVal.map(row => {
          if (init) {
            const rowIdx = lodash.findIndex(this.$refs.table.data, row);
            if (rowIdx >= 0) {
    ...
    return (
      ...
      >
        ...
        <el-table
          ...
          data={lodash.isArray(listData) ? listData : []}
        >
          {selectRow ? (
